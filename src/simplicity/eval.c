...
/* The program counter, 'pc', is the current combinator being interpreted.  */
  ...
  //initialize pc to last instruction (bottom of call stack)
  size_t pc = len - 1;
...
//EE
  ...
  //Set the return pointer of the current stack to the len ('an out-of-bounds index' as quoted from comment above)
  stack[pc].return_to = len;

  //Set the flag for tco to false?
  ...
  //While the program counter is at a valid place
  while(pc < len) {

    //Set the flags to exec
    stack[pc].flags |= FLAG_EXEC;

    //Get the tag of the node
    tag_t tag = dag[pc].tag;

    //Assert that the read frame iis away from the write frame
    assert(state.activeReadFrame < state.activeWriteFrame);

    //Assert the edge of the read is away from write
    assert(state.activeReadFrame->edge <= state.activeWriteFrame->edge);

    //If the node is a jet
    //CHECKPOINT
    if (dag[pc].jet) {
...
bool evalTCOExpression( bool *evalSuccess, flags_type anti_dos_checks, UWORD* output, size_t outputSize, const UWORD* input, size_t inputSize
  ...
  //If the eval TCO bound could not be computed then return false
  if (!computeEvalTCOBound(&bound, dag, type_dag, len)) return false;

//Add the input size to the output size and the larger of the extra cells bound
  ...
  //Make sure some values don't exceed a certain size and fail if they do
  ...
  //Allocate the cells to be the size of the cells bound if defined (each cell is a uword)
  UWORD* cells = calloc(cellsBound ? cellsBound : 1, sizeof(UWORD));

  //If the stackBound is less than or equal to size_max/sizeof(frameItem) (so if the max individual frameItem is bigger than stack bound)
  frameItem* frames = stackBound <= SIZE_MAX / sizeof(frameItem)
                    //allocate more space for the frames
                    ? malloc(stackBound * sizeof(frameItem))
                    //Otherwise if the stackbound is greater than frame, return null
                    : NULL;

  //Create call stack
  call* stack = calloc(len, sizeof(call));

  //put if current allocations into bool result
  const bool result = cells && frames && stack;

  //If everything is defined
  if (result) {

    //If the input is defined, copy the input into the allocated cells
    if (input) memcpy(cells, input, ROUND_UWORD(inputSize) * sizeof(UWORD));

    //Initialize state to use frames as read frame and frames + stackbound as write frames
    evalState state =
      { .activeReadFrame = frames
      , .activeWriteFrame = frames + (stackBound - 1)
      };

    //Initialize the read frame
    *(state.activeReadFrame) = initReadFrame(inputSize, cells);

    //Initialize the write frame
    *(state.activeWriteFrame) = initWriteFrame(outputSize, cells + cellsBound);

    //RunTCO and see what we get
    *evalSuccess = runTCO(state, stack, dag, type_dag, len, env);

    //If it was successful and we have an output
    if (*evalSuccess && output) {
      //Coput the active write frames edge to the output
