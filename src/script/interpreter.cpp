...
bool GenericTransactionSignatureChecker<T>::CheckSimplicity(const valtype& witness, const std::vector<unsigned char>& program, const rawTapEnv& simplicityRawTap, ScriptError* serror) const
{
    ...
    if (elements_simplicity_execSimplicity(&success, 0, txdata->m_simplicity_tx_data, nIn, simplicityTapEnv, txdata->m_hash_genesis_block.data(), 0, witness.data(), witness.size())) {
        free(simplicityTapEnv);
        return success ? set_success(serror) : set_error(serror, SCRIPT_ERR_EVAL_FALSE);
    }
